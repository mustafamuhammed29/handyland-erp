...
export function ReceiptPrintLayout({ repair, config }: { repair: any; config: ReceiptConfig }) {
  ...
  return (
    <div className="print-container bg-white w-full max-w-[800px] shadow-2xl relative overflow-hidden text-gray-900 mx-auto font-sans text-[12px] leading-tight" style={{ minHeight: "297mm", maxHeight: "297mm" }}>
      {/* Header */}
      <header className="bg-[#1a1a1a] text-white p-4 md:px-6 relative flex justify-between items-center overflow-hidden h-[90px]">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight m-0 leading-none">
            <span className="text-yellow-400">{c.shopNameMain}</span>{c.shopNameSub}
          </h1>
          <p className="text-xs md:text-sm uppercase tracking-widest mt-1 font-semibold text-gray-300">
            {c.headerSubtitle}
          </p>
        </div>
        
        {/* Logo / Icon */}
        <div className="relative z-10 w-14 h-14 bg-white rounded-full flex items-center justify-center p-1.5 border-2 border-gray-300 shrink-0 ml-4 overflow-hidden">
          {c.logoUrl ? (
      ...
      <main className="p-4 md:px-6 space-y-2">
        {/* Section 1: Personal Data */}
        <section>
          <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 mb-1">
            <h2 className="text-base font-bold uppercase tracking-wide m-0">{c.sec1Title}</h2>
            <span className="text-[10px] text-gray-600">{c.sec1Sub1}<span className="font-bold">{c.sec1Sub2}</span></span>
          </div>
          
          <div className="space-y-1.5">
            <div className="flex flex-col sm:flex-row sm:items-center">
              <label className="w-40 text-xs font-medium text-gray-700">{c.lblCustomerName}</label>
              <div className="border border-gray-600 bg-transparent px-2 min-h-[24px] flex-1 flex items-center font-bold text-xs">
                {repair?.customer ? `${repair.customer.firstName} ${repair.customer.lastName}` : ""}
              </div>
            </div>
            
            <div className="flex flex-col sm:flex-row sm:items-center">
              <label className="w-40 text-xs font-medium text-gray-700">{c.lblPhoneEmail}</label>
              <div className="border border-gray-600 bg-transparent px-2 min-h-[24px] flex-1 flex items-center font-bold text-xs">
                {repair?.customer ? `${repair.customer.phone || ""} / ${repair.customer.email || ""}` : ""}
              </div>
            </div>
            
            <div className="flex flex-col sm:flex-row sm:items-start pt-0.5">
              <label className="w-40 text-xs font-medium text-gray-700 pt-1">{c.lblPassword}</label>
              <div className="flex-1 flex gap-3 items-start">
                <div className="border border-gray-600 bg-transparent px-2 min-h-[28px] w-1/2 text-gray-500 relative flex items-center font-bold pt-2.5 text-xs">
                  <span className="absolute left-1 top-0.5 text-[7px] text-gray-400 font-normal">{c.lblPasswordSub}</span>
                  <div className="text-black leading-tight">{renderValue(repair?.devicePasswordEncrypted)}</div>
                </div>
                <div className="border border-gray-600 bg-transparent px-2 min-h-[28px] w-1/3 text-gray-500 relative flex items-center font-bold pt-2.5 text-xs">
                  <span className="absolute left-1 top-0.5 text-[7px] text-gray-400 font-normal">{c.lblSimPin}</span>
                  <div className="text-black leading-tight">{renderValue(repair?.simPinEncrypted)}</div>
                </div>
                
                <div className="border border-gray-600 bg-white relative overflow-hidden flex items-center justify-center" style={{ width: '40px', height: '40px', minWidth: '40px' }}>
        ...
        <section>
          <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 mb-1">
            <h2 className="text-base font-bold uppercase tracking-wide m-0">{c.sec2Title}</h2>
            <span className="text-[10px] text-gray-600">{c.sec2Sub}</span>
          </div>

          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <div className="flex-1 flex flex-col sm:flex-row sm:items-center">
              <label className="w-40 text-xs font-medium text-gray-700">{c.lblDevice}</label>
              <div className="border border-gray-600 bg-transparent px-2 min-h-[24px] flex-1 flex items-center font-bold text-xs">
            ...
            <div className="flex flex-col gap-1 min-w-[120px]">
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex gap-2">
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={isChecked(repair?.hasSimCard)} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.lblYes}</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={!isChecked(repair?.hasSimCard)} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.lblNo}</label>
                </div>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-700 mr-2">{c.lblCase}</span>
                <div className="flex gap-2">
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={isChecked(repair?.hasCase)} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.lblYes}</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={!isChecked(repair?.hasCase)} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.lblNo}</label>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Section 3: Protocol */}
        <section className="bg-gray-50 -mx-4 px-4 py-2 md:-mx-6 md:px-6 border-y border-gray-200">
          <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 mb-1">
            <h2 className="text-base font-bold uppercase tracking-wide m-0">{c.sec3Title}</h2>
            <span className="text-[10px] text-gray-600">{c.sec3Sub}</span>
          </div>
          ...
          <div className="space-y-2 text-xs">
            <div>
              ...
              <div className="border-b border-gray-600 mt-1 min-h-[20px] text-blue-800 italic font-medium px-1 overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{renderValue(repair?.previousRepairsDesc)}</div>
            </div>

            <div className="flex items-start gap-2">
              <p className="whitespace-nowrap">{c.txtCondBefore}<span className="font-bold">{c.txtCondBold}</span>{c.txtCondAfter}</p>
              <div className="border-b border-gray-600 flex-1 min-h-[20px] text-blue-800 italic font-medium px-1 overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{getConditionNotes()}</div>
            </div>

            <hr className="border-gray-300 my-2" />

            <div className="flex flex-col md:flex-row gap-2 py-0.5">
              <div className="w-40 font-medium text-gray-700">{c.lblDefects.split('\\n').map((line, i) => <React.Fragment key={i}>{line}<br/></React.Fragment>)}</div>
              <div className="flex-1 space-y-1.5">
                <div className="flex flex-wrap gap-2">
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={hasIssue('SPEAKER')} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defSpeaker}</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={hasIssue('EARPIECE')} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defEarpiece}</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={hasIssue('MICROPHONE')} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defMic}</label>
                </div>
                <div className="flex flex-wrap gap-2">
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={hasIssue('DISPLAY')} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defDisplay}</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={hasIssue('BACK_COVER')} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defBackCover}</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={hasIssue('BATTERY')} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defBattery}</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={hasIssue('CHARGING_PORT')} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defChargingPort}</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={hasIssue('WATER_DAMAGE')} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defWaterDamage}</label>
                </div>
                <div className="flex items-center gap-1">
                  <label className="flex items-center gap-1 cursor-pointer whitespace-nowrap"><input type="checkbox" checked={hasIssue('OTHER') || getOtherIssues() !== ""} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {c.defOther}</label>
                  <div className="border-b border-gray-600 flex-1 min-h-[20px] text-blue-800 italic font-medium px-1 overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{getOtherIssues()}</div>
                </div>
              </div>
            </div>

            <div className="flex flex-col md:flex-row gap-2 items-center mt-1">
              <div className="w-40 font-medium text-gray-700">{c.lblTimeEstimate.split('\\n').map((line, i) => <React.Fragment key={i}>{line}<br/></React.Fragment>)}</div>
                ...
                {["30min", "45min", "1h", "1.5h", "2h", "2.5h", "3h", "4h"].map((time, idx) => {
                  ...
                  return (
                    <label key={time} className="flex items-center gap-1 cursor-pointer">
                      <input type="checkbox" checked={repair?.repairTimeEstimate === time} className="w-3 h-3 border-gray-500 rounded-none accent-black" readOnly /> {displayTime}
                    </label>
                  )
                })}
              </div>
            ...
            <div className="flex flex-col md:flex-row gap-2 items-center mt-2">
              <div className="w-40 font-medium text-gray-700">{c.lblPickupDate}</div>
              <div className="flex-1">
                <div className="border border-gray-600 bg-white px-2 h-8 flex items-center font-bold text-xs">
            ...
            <div className="flex flex-col md:flex-row gap-2 mt-2">
              <div className="w-40 font-medium text-gray-700">{c.lblAdminSignature.split('\\n').map((line, i) => <React.Fragment key={i}>{line}<br/></React.Fragment>)}</div>
              <div className="flex-1 flex gap-2">
                <div className="flex-1 border border-gray-600 h-10 bg-white relative overflow-hidden flex items-center justify-center">
                ...
                <div className="w-24 border border-gray-600 h-10 bg-white p-1 flex flex-col justify-end relative shrink-0">
                  <span className="absolute left-1 top-0.5 text-[8px] text-gray-500">{c.lblPrice}</span>
                  <div className="w-full text-right outline-none font-bold text-base bg-transparent">{repair?.estimatedPrice ? `€ ${repair.estimatedPrice}` : ""}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="text-[9px] md:text-[10px] leading-tight text-gray-600 space-y-1 text-justify mt-2">
        ...
        <section className="flex gap-4 mt-2">
          <div className="w-32 flex flex-col gap-0.5 shrink-0">
            <label className="text-[10px] text-gray-700">{c.lblDate}</label>
            <div className="border border-gray-600 px-2 h-6 bg-gray-200 flex items-center font-bold text-xs">
              {repair?.createdAt ? format(new Date(repair.createdAt), "dd.MM.yyyy") : ""}
            </div>
          </div>
          <div className="flex-1 flex flex-col gap-0.5">
            <label className="text-[10px] text-gray-700 flex gap-2">
              {c.lblCustomerSignature}
              <span className="text-[9px] font-normal mt-0.5 text-gray-800 leading-tight">
                {c.txtCustSig1}<strong>{c.txtCustSigBold1}</strong><br/>
                {c.txtCustSig2}<strong>{c.txtCustSigBold2}</strong>
              </span>
            </label>
            <div className="border border-gray-600 h-10 bg-white w-full relative overflow-hidden flex items-center justify-center">
              ...
              <span className="absolute bottom-1 right-2 text-[7px] text-gray-400">{c.lblTicket} {repair?.ticketNumber || ""}</span>
            </div>
          </div>
        </section>
      </main>
      ...
      {/* Footer Banner */}
      <footer className="absolute bottom-0 w-full bg-[#1a1a1a] text-gray-300 text-center py-2 text-[9px] md:text-[10px] font-semibold tracking-wide uppercase">
        {c.footerText}
      </footer>
    </div>
  );
